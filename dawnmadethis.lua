...
local ragdolltestinglib = Instance.new("ScreenGui")
ragdolltestinglib.Name = "ragdolltestinglib"
ragdolltestinglib.Parent = game.CoreGui
ragdolltestinglib.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	...
	function(io, p)
		if io.KeyCode == Enum.KeyCode.RightShift then
			if uitoggled == false then
				ragdolltestinglib.Enabled = false
				uitoggled = true
			else
				ragdolltestinglib.Enabled = true
				uitoggled = false
			end
		end
	end
...
function lib:notification(textt)
	for i, v in next, ragdolltestinglib:GetChildren() do
	...
	notificationframe.Parent = ragdolltestinglib
...
function lib:MainWindow(text)
	...
	mainframe.Parent = ragdolltestinglib
